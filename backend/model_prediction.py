...
def train_models(X_train, y_train_temp, y_train_precipitation, y_train_humidity, y_train_wind_direction):
    ...
    joblib.dump(model_temp, 'backend/model_temp.pkl')
    ...
    joblib.dump(model_precipitation, 'backend/model_precip.pkl')
...
def save_predictions_to_json(predictions, future_date, file_name="backend/predicciones.json"):
...
def main():
    # Construir la ruta al archivo RAFstationdata.json
    file_path = './backend/RAFstationdata.json'
    
    # Cargar datos desde el archivo JSON
    df = load_data_from_json(file_path)
    
    # Si no se pudo cargar el archivo, salir del programa
    if df is None:
        return
    ...
    # Interfaz gráfica para la predicción meteorológica futura
    root = Tk()
